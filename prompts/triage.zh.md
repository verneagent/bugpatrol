...
5. `affected_branch` 是 bug 所在的分支，只能填 schema 中列出的 allowed branch patterns 匹配的具体分支名（如 `feature-*` 模式下填 `feature-login`）。优先根据 reporter 的自然语言描述、build 信息、截图中的版本号推断；注意 context 中的"Lark 话题更新"跟进消息可能就是 reporter 对"影响分支询问"的直接回答（内容往往只有一个分支名，如 `post`）。证据不足时输出空字符串，不要默认填 `main`。
