"""Reed-Solomon RS(255, 223) codec unit tests.

The screenshot pixel carrier splits its envelope across 5 RS(255,223) blocks
(NSYM=32), so each block tolerates up to 16 corrupted bytes before the whole
envelope is lost. These tests pin the codec's error-correction contract and
field conventions (mirrored from the canonical reedsolo library) — both the
app-side TypeScript encoder and the BugPatrol Python decoder share them.
"""
...
class Rs256CodecTest(unittest.TestCase):
    ...
    def test_corrects_up_to_16_byte_errors_per_block(self) -> None:
        # t = nsym/2 = 16: the codec MUST correct any 0..16 corrupted bytes.
        code = self._code(3)
