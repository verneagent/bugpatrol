...
class FixNotifyTest(unittest.TestCase):
    ...
    def test_notify_fix_write_records_resolution_field(self) -> None:
        config = load_project_config(Path("projects/todo-sandbox.toml"))
        github = FakeGitHubIssuesClient()
        lark = FakeLarkMessengerClient()
        ...
        self.assertTrue(summary.resolution_written)
        self.assertEqual(len(issue_fields.calls), 1)
        self.assertEqual(issue_fields.calls[0]["values"], {"Resolution": "代码修复"})

        duplicate = apply_fix_notification(
            repo=config.github_repo,
            issue_number=issue.number,
            event="pr_merged",
            pr="123",
        ...
        self.assertFalse(duplicate.resolution_written)
        self.assertEqual(len(issue_fields.calls), 1)

    def test_notify_fix_pr_opened_does_not_write_resolution(self) -> None:
        config = load_project_config(Path("projects/todo-sandbox.toml"))
        github = FakeGitHubIssuesClient()
        lark = FakeLarkMessengerClient()
        issue = self._managed_issue(config, github, lark)
        ...
        self.assertFalse(summary.resolution_written)
        self.assertEqual(issue_fields.calls, [])

    def _managed_issue(self, config, github, lark):
        workflow = IntakeWorkflow(config=config, github=github, lark=lark)
